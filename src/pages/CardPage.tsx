...
// Social icons for back of card (larger)
const socialIconsLarge: Record<string, ReactNode> = {
  linkedin: (
...
function getAccent(theme?: CardTheme) {
  return theme?.primaryColor || GOLD;
}
...
export function CardPage({ username, slug }: CardPageProps) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-neutral-100">
        <div className="w-7 h-7 border-2 border-neutral-300 border-t-neutral-600 rounded-full animate-spin" />
      </div>
    );
  }
  ...
  const accent = getAccent(card.theme);
  const fontFamily = card.theme?.fontFamily || 'Inter, system-ui, sans-serif';
    ...
    >
      <div className="w-full max-w-sm">
          ...
          >
                  ...
                  <div className="relative h-full flex flex-col justify-between p-5 sm:p-6">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="min-w-0 pt-0.5">
                        <h1 className="text-white font-semibold text-[17px] leading-tight tracking-tight truncate">
                          {card.displayName}
                        </h1>
                        {card.title && (
                          <p
            ...
            >
              {card.htmlBackContent ? (
                <div
                  className="w-full h-full"
                  dangerouslySetInnerHTML={{ __html: card.htmlBackContent }}
                />
              ) : (
                /* Fallback: React rendering for pre-migration cards */
                <div
                      ...
                      <div className="flex items-center gap-3 mb-3">
                        {socialLinks.map((link, i) => (
                          <a
                            ...
                            style={{ color: accent }}
                          >
                            {socialIconsLarge[link.type]}
                          </a>
                        ))}
                      </div>
                    )}

                    {/* Bottom row: website + flip hint */}
                    <div className="flex items-end justify-between">
        ...
        >
          ...
          {/* Links (non-social, since social are on card back) */}
          {otherLinks.length > 0 && (
            <>
              <div className="h-px bg-neutral-50 mx-4" />
          ...
          {/* Also show social links in detail section for accessibility */}
          {socialLinks.length > 0 && (
            <>
              <div className="h-px bg-neutral-50 mx-4" />
              <div className="px-4 py-3">
        ...
        {/* Footer */}
        <p className="text-center text-[10px] mt-6 tracking-widest uppercase text-neutral-400">
          Powered by Calendar
        </p>
      </div>
